...
# 12. XGBoost 模型
#
# 網站啟動時由後台自動訓練 XGBoost。
# 使用者不需要看到模型訓練頁面。
#
# @st.cache_resource：
# 第一次啟動網站時訓練一次。
# Streamlit rerun 時會直接使用已訓練好的模型，
# 不會因為使用者重新輸入資料而再次訓練。
# ============================================================
...
# ============================================================
    ...
    st.write(
        ...
        """
        本專題以Kaggle:Gaming & Mental Health 資料集為基礎，
...
低／偏低／中／偏高／高
    ...
    st.write(
        "XGBoost 模型會在網站啟動時由後台自動訓練。"
        "第一次啟動時完成模型訓練後，會透過 Streamlit Cache 保留模型；"
        "使用者輸入 14 項資料並按下開始分析後，系統直接執行模型預測。"
        "網站重新執行時不會因為使用者操作而重複訓練模型。"
    )

    st.subheader("📈 模型評估")

    if model_rmse is not None and model_mae is not None and model_r2 is not None:

        c1, c2, c3 = st.columns(3)

        with c1:
            st.metric("RMSE", f"{model_rmse:.4f}")

        with c2:
            st.metric("MAE", f"{model_mae:.4f}")

        with c3:
            st.metric("R²", f"{model_r2:.4f}")
    ...
    model_setting_df.index = range(1, len(model_setting_df) + 1)

    st.dataframe(model_setting_df, use_container_width=True)
